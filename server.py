...
#  server = Server(np.unique(model_names), test_loader,test_loader,num_classes=num_classes, images_train=images_train,
#  labels_train=labels_train, eta=hp.get('eta', 0) , dataset = hp['dataset'])
class Server(Device):
  def __init__(self, model_names, loader, val_loader, num_classes=10,imsize=None, images_train=None, labels_train=None, eta=0.5 , dataset = 'cifar10', client_loaders=None):
    ...
    self.model_dict = {model_name : partial(model_utils.get_model(model_name)[0], num_classes=num_classes,im_size=imsize, dataset = dataset)().to(device) for model_name in model_names}

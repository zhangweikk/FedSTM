...
class Client(Device):
  def __init__(self, model_name, optimizer_fn, loader, idnum=0, num_classes=10,imsize=None, images_train=None, labels_train=None, eta=0.5, dataset = 'cifar10'):
    super().__init__(loader)
    self.id = idnum
    print(f"dataset client {dataset}")
    self.model_name = model_name
   
    self.model_fn = partial(model_utils.get_model(self.model_name)[0], num_classes=num_classes ,im_size=imsize, dataset = dataset)
    ...
    for _, labels in self.loader:
        for label in labels:
            self.cls_num_list[label.item()] += 1
  ...
  def compute_weight_update(self, epochs=1, loader=None, lambda_fedprox=0.0, print_train_loss=False,  hp=None, args=None):
    ...
    if privacy_sigma is not None:
      ...
      if args.use_feddecor:
        # print("using feddecor")
        train_stats = train_op_feddecor(
          self.model,
          self.loader if not loader else loader,
          self.optimizer,
          epochs,
          lambda_fedprox=lambda_fedprox,
          print_train_loss=print_train_loss,
          feddecorr_coef = args.feddecorr_coef
        )
      else:
